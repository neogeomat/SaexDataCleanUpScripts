            ...
            try:
                ...
                if(no_of_attribute==0):
                    ...
                    arcpy.FeatureClassToFeatureClass_conversion(Data_Location + "\\Parcel", DataCleanTemp, "Parcel.shp", "",
                                                                ...
                                                                + Data_Location + "\\Parcel,Shape_Area,-1,-1;Mother_parcel \"Mother_parcel\" true true false 255 Text 0 0 ,First,#,"
                                                                + Data_Location + "\\Parcel,Mother_parcel,-1,-1", "")
                    ...
                    arcpy.SpatialJoin_analysis(DataCleanTemp + "\\CleanPoly.shp", DataCleanTemp + "\\ParcelCentroid.shp",
                                               ...
                                               + DataCleanTemp + "\\ParcelCentroid.shp,ORIG_FID,-1,-1;Mother_par \"Mother_par\" true true false 255 Text 0 0 ,First,#,"
                                               + DataCleanTemp + "\\ParcelCentroid.shp,Mother_par,-1,-1", "CONTAINS", "", "")
                    ...
                    parcel_fc = Data_Location + "\\Parcel"
                    parcel1_fc = DataCleanTemp + "\\Parcel1.shp"

                    # Define the field mapping between the input and output feature classes
                    field_mappings = arcpy.FieldMappings()
                    field_mappings.addTable(parcel_fc)
                    field_mappings.addTable(parcel1_fc)
                    mother_parcel_map = arcpy.FieldMap()
                    mother_parcel_map.addInputField(parcel1_fc, "mother_par")
                    mother_parcel_output = mother_parcel_map.outputField
                    mother_parcel_output.name = "mother_parcel"
                    mother_parcel_map.outputField = mother_parcel_output
                    field_mappings.addFieldMap(mother_parcel_map)

                    # Append the input feature class to the output feature class, mapping the fields as necessary
                    arcpy.Append_management(parcel1_fc, parcel_fc, "NO_TEST", field_mappings)
