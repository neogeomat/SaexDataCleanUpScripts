...
class Downloader:
    ...
    @classmethod
    def __get_branch_from_url(cls, url, branch=''):
    ...
    @classmethod
    def __get_raw_url(cls, file_path, url, branch=''):
        tmp_url = url.replace(
    ...
    def download(
        ...
        if target_folder == '*':
            ...
            start = self.location.get(target_folder, 0)
        ...
        for i in self.files[start:]:
            if recursive or i[0].split(target_folder)[1].count('/') \
                    <= 1:
                self.__mkdirs(os.path.join(destination, os.path.dirname(i[0])))
                urllib.urlretrieve(i[1], os.path.join(destination, i[0]))
