...
def merge_dummy_planning(self, choosen_meridian, status_update=None, show_messagebox=True, update_progress=None):
    ...
    for progress, i in enumerate(mdb_list, start=1):
        ...
        try:
            ...
            arcpy.CopyFeatures_management(blank_mdb + "\\Parcel", Data_Location + "\\Parcel")
            arcpy.Append_management(DataCleanTempFile + "\\NewJoinedData.shp", Data_Location + "\\Parcel", "NO_TEST")
            arcpy.Append_management(DataCleanTempFile + "\\ParcelRemain.shp", Data_Location + "\\Parcel", "NO_TEST")
